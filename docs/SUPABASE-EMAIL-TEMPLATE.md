...
MandiBook Lab uses **email OTP** login. By default Supabase sends only a **magic link** (no 6-digit code). Update the template once in your Supabase dashboard.

## Steps

1. Open **Supabase Dashboard → Authentication → Email Templates**
2. Select **Magic Link** (this template is used for `signInWithOtp` email)
...
<p>Apna 6-digit code enter karein:</p>
...
2. Either enter the **6-digit code** on the login page, **or** click the **Login karein** link
...
| Problem | Fix |
|---------|-----|
| Email shows only "Sign in" link, no code | Template not saved — paste HTML above |
